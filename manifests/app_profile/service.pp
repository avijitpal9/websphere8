...
define websphere8::app_profile::service (
  $type,
  $profile_base,
  $profile_name = $title,
  $server_name  = 'server1',
  ...
  validate_re($type, '(dmgr|appserver|node)')
  ...
  if ! $status {
    ...
    } elsif $type == 'appserver' {
      $_status = "/bin/su - ${user} -c '${profile_base}/${profile_name}/bin/serverStatus.sh ${server_name} -username '${wsadmin_user}' -password '${wsadmin_pass}' -profileName ${profile_name} | grep -q STARTED'"
    } else {
  ...
  if ! $start {
    if $type == 'dmgr' {
      ...
      $_start = "/bin/su - ${user} -c '${profile_base}/${profile_name}/bin/startServer.sh ${server_name} -profileName ${profile_name} -username ${wsadmin_user} -password ${wsadmin_pass}'"
    } else {
  ...
  if ! $stop {
    if $type == 'dmgr' {
      $_stop = "/bin/su - ${user} -c '${profile_base}/${profile_name}/bin/stopManager.sh -profileName ${profile_name} -username ${wsadmin_user} -password ${wsadmin_pass}'"
        ...
        $_stop = "/bin/su - ${user} -c '${profile_base}/${profile_name}/bin/stopServer.sh ${server_name} -profileName ${profile_name} -username ${wsadmin_user} -password ${wsadmin_pass}'"
    } else {
